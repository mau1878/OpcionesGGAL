...
st.title("Estrategias Bearish Spreads")
...
if 'selected_visualizations_bear_put' not in st.session_state:
    st.session_state.selected_visualizations_bear_put = []
...
st.sidebar.button("Limpiar Visualizaciones", on_click=lambda: st.session_state.update({
    "selected_visualizations_bear_call": [], "selected_visualizations_bear_put": []
}))
...
detailed_df_bear_call = utils.create_bearish_spread_table(filtered_calls, utils.calculate_bear_call_spread, num_contracts, commission_rate, is_debit=False)

def visualize_callback_bear_call():
    logger.info("Visualize callback triggered for Bear Call Spread")
    edited = st.session_state.get("bear_call_spread_editor_unique", {})
    edited_rows = edited.get('edited_rows', {})
    edited_df_local = st.session_state["bear_call_df"]
    for idx in edited_rows:
        if isinstance(idx, int) and 0 <= idx < len(edited_df_local):
            ...
            if visualize_state and idx not in st.session_state["selected_visualizations_bear_call"] and len(st.session_state["selected_visualizations_bear_call"]) < 3:
                st.session_state["selected_visualizations_bear_call"].append(idx)
            elif not visualize_state and idx in st.session_state["selected_visualizations_bear_call"]:
                st.session_state["selected_visualizations_bear_call"].remove(idx)
    
    for idx in st.session_state["selected_visualizations_bear_call"]:
        ...
        if long_opt and short_opt:
            ...
            iv_calibrated = utils._calibrate_iv(
                ...
                lambda p, t, s: (utils.black_scholes(p, short_opt["strike"], t, st.session_state.risk_free_rate, s, "call") -
                                 utils.black_scholes(p, long_opt["strike"], t, st.session_state.risk_free_rate, s, "call")),
                options, actions
            )
            ...
            utils.visualize_bearish_3d(
                ...
                iv_calibrated, "Bear Call Spread", options, actions
            )
            ...
            if show_2d_plot:
                ...
                fig_2d.update_layout(
                    title="P&L at Expiration for Bear Call Spread",
                ...
                st.plotly_chart(fig_2d, use_container_width=True, key=f"bear_call_2d_plot_{idx}_{id(options)}")
        else:
...
if not detailed_df_bear_call.empty:
    ...
    st.warning("No hay combinaciones válidas para Bear Call Spread con los filtros actuales.")

# Bear Put Spread
st.header("Bear Put Spread (Debit Spread)")
filtered_puts = [opt for opt in puts if min_strike <= opt["strike"] <= max_strike]
detailed_df_bear_put = utils.create_bearish_spread_table(filtered_puts, utils.calculate_bear_put_spread, num_contracts, commission_rate, is_debit=True)

def visualize_callback_bear_put():
    logger.info("Visualize callback triggered for Bear Put Spread")
    edited = st.session_state.get("bear_put_spread_editor_unique", {})
    edited_rows = edited.get('edited_rows', {})
    edited_df_local = st.session_state["bear_put_df"]
    for idx in edited_rows:
        if isinstance(idx, int) and 0 <= idx < len(edited_df_local):
            visualize_state = edited_rows[idx].get('Visualize', False)
            if visualize_state and idx not in st.session_state["selected_visualizations_bear_put"] and len(st.session_state["selected_visualizations_bear_put"]) < 3:
                st.session_state["selected_visualizations_bear_put"].append(idx)
            elif not visualize_state and idx in st.session_state["selected_visualizations_bear_put"]:
                st.session_state["selected_visualizations_bear_put"].remove(idx)
    
    for idx in st.session_state["selected_visualizations_bear_put"]:
        row = edited_df_local.iloc[idx]
        result = {
        ...
        if long_opt and short_opt:
            ...
            iv_calibrated = utils._calibrate_iv(
                ...
                current_price,
                st.session_state.expiration_days,
                lambda p, t, s: (utils.black_scholes(p, long_opt["strike"], t, st.session_state.risk_free_rate, s, "put") -
                                 utils.black_scholes(p, short_opt["strike"], t, st.session_state.risk_free_rate, s, "put")),
                options, actions
            )
            if iv_calibrated == utils.DEFAULT_IV:
                st.warning(f"No se pudo calibrar la volatilidad implícita para {long_opt['symbol']}/{short_opt['symbol']}. Usando valor predeterminado.")
            ...
            utils.visualize_bearish_3d(
                result, current_price, st.session_state.expiration_days,
                iv_calibrated, "Bear Put Spread", options, actions
            )
            ...
            if show_2d_plot:
                ...
                fig_2d.update_layout(
                    title="P&L at Expiration for Bear Put Spread",
                    xaxis_title="Precio de GGAL (ARS)",
                    yaxis_title="P&L (ARS)",
                    xaxis=dict(tickvals=[min_price, current_price, max_price],
                               ticktext=[f"{min_price:.2f}", f"{current_price:.2f}", f"{max_price:.2f}"]),
                ...
                st.plotly_chart(fig_2d, use_container_width=True, key=f"bear_put_2d_plot_{idx}_{id(options)}")
        else:
            st.warning("Datos de opciones no disponibles para esta combinación.")
    st.rerun()
...
if not detailed_df_bear_put.empty:
    ...
    st.warning("No hay combinaciones válidas para Bear Put Spread con los filtros actuales.")