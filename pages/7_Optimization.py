...
max_legs = st.sidebar.number_input("Máximo número de patas", min_value=1, max_value=3, value=2, step=1)
...
if 'selected_visualizations_opt' not in st.session_state:
    st.session_state.selected_visualizations_opt = []

total_options = calls + puts
if len(total_options) > 20:
    total_options = random.sample(total_options, 20)
    st.warning("Muestreando opciones para mejorar rendimiento.")

results = []
min_price = current_price * (1 - st.session_state.plot_range_pct)
max_price = current_price * (1 + st.session_state.plot_range_pct)
with st.spinner("Optimizando estrategias..."):
    for num_legs in range(1, max_legs + 1):
        for opt_combinations in combinations(total_options, num_legs):
            for actions in product(['buy', 'sell'], repeat=num_legs):
    ...
    def visualize_callback():
        ...
        for idx in edited_rows:
            if isinstance(idx, int) and 0 <= idx < len(df):
                ...
                if visualize_state and idx not in st.session_state["selected_visualizations_opt"] and len(st.session_state["selected_visualizations_opt"]) < 5:
                    st.session_state["selected_visualizations_opt"].append(idx)
                elif not visualize_state and idx in st.session_state["selected_visualizations_opt"]:
                    st.session_state["selected_visualizations_opt"].remove(idx)
        
        for idx in st.session_state["selected_visualizations_opt"]:
            row = df.iloc[idx]
            options = row["Options"]
            actions = row["Actions"]
